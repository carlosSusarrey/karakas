...
function applyAction(state: GameState, action: GameAction): GameState {
  switch (action.type) {
    ...
    case "SET_MONARCH": {
      return {
        ...state,
        players: state.players.map((p) => ({ ...p, isMonarch: p.id === action.playerId })),
      };
    }
    ...
    case "SET_INITIATIVE": {
      return {
        ...state,
        players: state.players.map((p) => ({ ...p, hasInitiative: p.id === action.playerId })),
      };
    }
    ...
    case "REINSTATE_PLAYER": {
      return {
        ...state,
        players: state.players.map((p) =>
          p.id === action.playerId ? { ...p, isEliminated: false, eliminatedTurn: null } : p,
        ),
      };
    }
    ...
    case "ADVANCE_TURN": {
      ...
      const newTurn =
        nextIndex === firstActiveIndex ? state.currentTurn + 1 : state.currentTurn;
      return { ...state, currentTurn: newTurn, activePlayerIndex: nextIndex };
    }
